"""HTTP handler for the scheduler LLM gateway (vLLM OpenAI API only)."""
...
from local_model_hints import DEFAULT_HUB_REPO, resolve_context_token_limit  # noqa: E402
...
_FORMAT_REPAIR_SYSTEM = (
    "You repair day-scheduler assistant replies. Output ONLY the final user-visible reply matching "
    "the opener contract: first line MUST be EITHER a Markdown ATX heading (`#`/`##`), OR the first "
    "line of a fenced code block opened with triple backticks tagged `schedule` or `plan`, OR the "
    "legacy Unicode timetable frame (`╭` …). Follow with timetable task bullets, then prose. "
    "No scratchpad or thinking tags; keep intent; normalize formatting strictly."
)
...
def build_llm_gateway_handler(
    *,
    ...
    vllm_context: VllmGatewayContext,
):
    ...
    def _run_format_repair_completion(
        *,
        ...
        user_block = (
            ...
            "Required **opener** (first substantive line is exactly ONE of):\n"
            "- Markdown `#`/`## …` timetable heading **or**\n"
            "- First line opens a fenced triple-backtick block tagged `schedule`/`plan`, **or**\n"
            "- Legacy Unicode frame beginning with ╭ …\n"
            "**Then task rows**\n"
    ...
    class SchedulerLlmGatewayHandler(BaseHTTPRequestHandler):
        ...
        def do_POST(self) -> None:  # noqa: N802
            ...
            if path == "/v1/plain-completion":
                ...
                temp_pc, top_pc, _ = _finalize_sampling_defaults_ui(
            ...
            sched_model_str = _resolve_scheduler_model(model_arg)
            ...
            if (
                ...
                _flow_log(
                    ...
                    lane="pipeline",
                    model=sched_model_str,
                )
                fixed = _run_format_repair_completion(
                    log_model=sched_model_str,
            ...
            _flow_log(
                flow_id,
                f"[scheduler] quality gate — generation_ok={primary_result.get('ok')} "
            ...
            generation_ok = bool(primary_result.get("ok"))
            response_ok = generation_ok and quality_ok
            chat_line = (
            ...
            _flow_log(
                ...
                (
                    ...
                    f"generation_ok={generation_ok} quality_ok={response_ok} "
...
def add_gateway_argparse(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        default=None,
        help=(
            "Tokenizer id for scheduler (local snapshot or Hub; env SCHEDULER_MODEL / MLX_MODEL; "
